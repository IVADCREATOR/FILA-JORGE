...
describe("applications: criação, API keys e isolamento", () => {
  ...
  it("issueKeyForExistingApplication emite key normalmente para aplicação existente", () => {
    const { application } = service.create({ name: "Issue Key App", scopes: [] });
    const rawKey = service.issueKeyForExistingApplication(application.id, ["memory:write"]);

    expect(rawKey).toMatch(/^sk_[a-f0-9]{64}$/);
    const authResult = service.authenticateByKey(rawKey);
    expect(authResult?.scopes).toEqual(["memory:write"]);
  });

  it("issueKeyForExistingApplication rejeita application_id inexistente em vez de deixar o SQLite quebrar por FK", () => {
    // Correção da auditoria: antes disso, isso lançava um erro cru de
    // FOREIGN KEY constraint do driver. Agora é um erro de domínio limpo.
    expect(() => service.issueKeyForExistingApplication(999999, [])).toThrow(/não encontrada/);
  });
});
